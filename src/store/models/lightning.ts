...
const lightningModel: LightningModel = {
  ...
  connectAllPeers: thunk(async (actions, network, { injections, getState }) => {
    ...
    for (const node of network.nodes.lightning) {
      ...
      try {
        ...
        info(`Failed to connect peers for '${node.name}': ${err.message}`);
      }
    }
  }),
  depositFunds: thunk(
  ...
  createInvoice: thunk(
    async (actions, { node, amount, memo, expiry }, { injections }) => {
      const api = injections.lightningFactory.getService(node);
      return await api.createInvoice(node, amount, memo, expiry);
    },
  ),
  payInvoice: thunk(
    async (
