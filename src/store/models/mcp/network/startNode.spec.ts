...
describe('MCP model > startNode', () => {
  ...
  it('should start a Bitcoin node', async () => {
  ...
  it('should start a Lightning node', async () => {
    const toggleNodeSpy = jest.spyOn(store.getActions().network, 'toggleNode');
    toggleNodeSpy.mockImplementation(async node => {
      // Simulate the toggleNode behavior by changing the node status
      const network = store
  ...
  it('should start a node that is in Error state', async () => {
    const toggleNodeSpy = jest.spyOn(store.getActions().network, 'toggleNode');
    toggleNodeSpy.mockImplementation(async node => {
      // Simulate the toggleNode behavior by changing the node status
      const network = store
  ...
  it('should throw error when networkId is not provided', async () => {
    await expect(store.getActions().mcp.startNode({} as any)).rejects.toThrow(
  ...
  it('should throw error when nodeName is not provided', async () => {
    await expect(
      store.getActions().mcp.startNode({
        networkId: 1,
      } as any),
  ...
  it('should throw error when node is already started', async () => {
    // Create a network first
    await store.getActions().network.addNetwork({
      name: 'test-network',
      description: 'Test',
    ...
    // Change node status to Started
    const network = store.getState().network.networks[0];
    network.nodes.bitcoin[0].status = Status.Started;
    const nodeName = network.nodes.bitcoin[0].name;
    ...
    await expect(
      ...
      `Cannot start node "${nodeName}". Node is currently Started. Only Stopped or Error nodes can be started.`,
    );
  });
  ...
  it('should throw error when node is starting', async () => {
    ...
    await expect(
      ...
      `Cannot start node "${nodeName}". Node is currently Starting. Only Stopped or Error nodes can be started.`,
    );
  });
  ...
  it('should throw error when node is stopping', async () => {
    ...
    await expect(
      ...
      `Cannot start node "${nodeName}". Node is currently Stopping. Only Stopped or Error nodes can be started.`,
    );
  });
});
