...
const SimulationDesignerTab: React.FC<Props> = ({ network }) => {
  ...
  const showRemoveModal = (networkId: number, activityId: number) => {
    ...
    modal = Modal.confirm({
      ...
      onOk: async () => {
        if (!network?.simulation) return;
        await removeSimulation({ id: activityId, networkId: network.id });
  ...
  const items: MenuProps['items'] = [
    { key: 'delete', label: 'Delete', icon: <CloseOutlined />, disabled: started },
  ];
      ...
      <Styled.Title>
        ...
        <Tooltip overlay={l('createBtn')} placement="topLeft">
          <Button
            ...
            icon={<PlusOutlined />}
            onClick={addSimulation}
            disabled={loading || network.status !== Status.Started}
          />
        </Tooltip>
      </Styled.Title>
      {cmp}
