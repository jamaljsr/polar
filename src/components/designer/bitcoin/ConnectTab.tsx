...
const ConnectTab: React.FC<Props> = ({ node }) => {
  ...
  if (isBtcd) {
    ...
    details = [
      // btcd and btcwallet both serve JSON-RPC over TLS, so the hosts are https
      { label: l('rpcHost'), value: `https://127.0.0.1:${node.ports.rpc}` },
      { label: l('walletRpcHost'), value: `https://127.0.0.1:${node.ports.btcdWallet}` },
