...
const Peer: React.FC<Props> = ({ from, to }) => {
  const { l } = usePrefixedTranslation('cmps.designer.link.Peer');
  ...
  return (
    <SidebarCard title={l('title')}>
      <p>{l('desc')}</p>
      <DetailsList title={l('peer1Title')} details={fromDetails} />
      <DetailsList title={l('peer2Title')} details={toDetails} />
    </SidebarCard>
  );
};
