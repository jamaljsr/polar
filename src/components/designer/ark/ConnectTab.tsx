...
import { Status, ArkdNode, ArkNode } from 'shared/types';
...
const ConnectTab: React.FC<Props> = ({ node }) => {
  const { l } = usePrefixedTranslation('cmps.designer.ark.ConnectTab');
  ...
  const info = useMemo((): ConnectionInfo => {
    if (node.status === Status.Started) {
      if (node.implementation === 'arkd') {
        const arkd = node as ArkdNode;
        return {
          restUrl: `https://127.0.0.1:${arkd.ports.api}`,
          grpcUrl: `127.0.0.1:${arkd.ports.api}`,
          apiDocsUrl: 'https://lightning.engineering/api-docs/api/ark/',
          credentials: {
            admin: arkd.paths.macaroon,
            cert: arkd.paths.tlsCert,
          },
        };
      }
    }
