...
describe('UnlockNodeModal', () => {
  ...
  describe('when the wallet state request fails', () => {
    ...
    it('should display an error instead of the password form', async () => {
      ...
      expect(queryByLabelText('Wallet Password')).not.toBeInTheDocument();
    });

    it('should not submit either wallet RPC while the state is unknown', async () => {
    ...
    it('should retry the request when retry is clicked', async () => {
      const { findByText, getByText, findByLabelText } = renderComponent();
      await findByText('Unable to determine the wallet state');
      lndServiceMock.getWalletState.mockResolvedValue('NON_EXISTING');
      fireEvent.click(getByText('Retry'));
      expect(await findByLabelText('Wallet Password')).toBeInTheDocument();
  ...
  describe('with wallet NON_EXISTING', () => {
    ...
    it('should keep the seed visible even if a wallet-state re-query rejects', async () => {
      lndServiceMock.genSeed.mockResolvedValue(['abandon', 'ability']);
      lndServiceMock.initWallet.mockResolvedValue(Buffer.from('admin-macaroon'));
      lightningServiceMock.waitUntilOnline.mockResolvedValue();
      ...
      fireEvent.click(getByText('Initialize'));
      expect(await findByText('Wallet Seed for alice')).toBeInTheDocument();
      // a slow re-query (triggered by monitorStartup) rejecting must not clobber the seed
      lndServiceMock.getWalletState.mockRejectedValue(new Error('getState-error'));
      expect(queryByText('Wallet Seed for alice')).toBeInTheDocument();
      expect(queryByText('Unable to determine the wallet state')).not.toBeInTheDocument();
      expect(getByText(/1\. abandon/)).toBeInTheDocument();
    });

    it('should report initError when the state query resolves mid-flight', async () => {
      lndServiceMock.genSeed.mockRejectedValue(new Error('test-error'));
      const { findByText, getByText, queryByText } = renderComponent();
      await findByText('Initialize Node alice');
      fireEvent.click(getByText('Initialize'));
      // the wallet state flips after submit and before genSeed settles; the modal
      // must still report the init-time error, not the unlock error
      lndServiceMock.getWalletState.mockResolvedValue('LOCKED');
      expect(await findByText('Unable to initialize the wallet')).toBeInTheDocument();
      expect(getByText('test-error')).toBeInTheDocument();
      expect(queryByText('Unable to unlock the wallet')).not.toBeInTheDocument();
    });
  });
});
