...
describe('Network Utils', () => {
  ...
  describe('createNetwork with bitcoind-knots', () => {
    ...
    it('should use latest bitcoind-knots version when LND target is not in knots versions', () => {
      const network = createNetwork({
        id: 1,
        name: 'knots-lnd-test',
    ...
    it('should adjust bitcoind-knots version for LND compatibility when target version is available', () => {
      ...
      const modifiedManagedImages: ManagedImage[] = [
        ...testManagedImages,
        { implementation: 'bitcoind-knots', version: '27.1', command: '' },
        { implementation: 'LND', version: '0.18.3-beta', command: '' },
      ];
      const network = createNetwork({
        id: 1,
        name: 'knots-lnd-compat-test',
        description: 'test knots with lnd compat',
        lndNodes: 1,
        clightningNodes: 0,
        eclairNodes: 0,
        bitcoindNodes: 0,
        ...
        litdNodes: 0,
        status: Status.Stopped,
        repoState: modifiedRepoState,
        managedImages: modifiedManagedImages,
        customImages: [],
        manualMineCount: 6,
      });
      expect(network.nodes.bitcoin).toHaveLength(1);
      expect(network.nodes.bitcoin[0].implementation).toBe('bitcoind-knots');
      expect(network.nodes.bitcoin[0].version).toBe('27.1');
    });
  });
