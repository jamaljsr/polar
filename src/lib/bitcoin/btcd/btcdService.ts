...
class BtcdService implements BitcoinService {
  async createDefaultWallet(node: BitcoinNode) {
    ...
    await httpPost<any>(node, body);

    // Get a mining address from the wallet if one hasn't been set yet.
    // btcd requires --miningaddr at startup; we store it on the node so the
    // compose file can include it on the next (and all future) starts.
    const btcdNode = node as BtcdNode;
    if (!btcdNode.miningAddr) {
      btcdNode.miningAddr = await this.getNewAddress(node);
      logger.debug(
        `Set mining address for btcd node ${node.name}: ${btcdNode.miningAddr}`,
      );
    }
  }
  ...
  async getBlockchainInfo(node: BitcoinNode) {
    ...
    return {
      ...
      bip9_softforks: result.bip9_softforks as any,
      ...
      prune_target_size: 0,
      softforks: result.bip9_softforks as any,
    };
  }
  ...
  async getWalletInfo(node: BitcoinNode) {
    ...
    const totalRecvPayload = {
      jsonrpc: '1.0',
      method: 'getreceivedbyaccount',
      params: ['default'],
    };
    const { result: totalRecv } = await httpPost<BTCD.GetTotalReceivedByAccountResponse>(
      node,
      totalRecvPayload,
    );

    const unspentPayload = {
      jsonrpc: '1.0',
      method: 'listunspent',
      params: [0, 0], // minconf=0, maxconf=0
    };
    const { result: unspent } = await httpPost<BTCD.ListUnspentResponse>(
      node,
      unspentPayload,
    );

    const unconfirmedBalance = unspent.reduce((acc, u) => acc + u.amount, 0);
    ...
    return {
      ...
      immature_balance: totalRecv - result.balance,
      txcount: unspent.length, // not very useful, so we just set it to the number of unspent outputs.
  ...
  /**
   * Mines a specified number of blocks on the given node.
   * @param numBlocks The number of blocks to mine.
   * @param node The node to mine on.
   * @returns The result of the mine operation.
   */
  ...
  private async mineUntilMaturity(node: BitcoinNode) {
    ...
    const { result } = await httpPost<BTCD.ListUnspentResponse>(node, listTrxBody);

    // determine the highest # of confirmations of all utxos. this is
    // the utxo we'd like to spend from
    const confs = Math.max(0, ...result.map((u: BTCD.Unspent) => u.confirmations));
