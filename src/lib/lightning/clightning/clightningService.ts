...
export class CLightningService implements LightningService {
  ...
  async waitUntilOnline(
    ...
    return waitFor(
      ...
      if (isWindows()) {
        await this.copyCertsFromContainer(this.cast(node));
      }
    });
  }
  ...
  /**
   * On Windows, CLN's data directory uses a named Docker volume instead of a
   * bind mount (to avoid POSIX fsync/rename failures on 9p/virtiofs). This means
   * gRPC TLS certs generated by CLN inside the container are not automatically
   * visible on the host filesystem. This method copies them out using docker cp.
   */
  private async copyCertsFromContainer(node: CLightningNode) {
    const log = (...args: any[]) => debug(`CLightningService ${node.name}:`, ...args);

    const certFiles = [
      {
        containerPath: '/home/clightning/.lightning/regtest/ca.pem',
        hostPath: node.paths.tlsCert,
      },
      {
        containerPath: '/home/clightning/.lightning/regtest/client.pem',
        hostPath: node.paths.tlsClientCert,
      },
      {
        containerPath: '/home/clightning/.lightning/regtest/client-key.pem',
        hostPath: node.paths.tlsClientKey,
      },
    ];

    const name = getContainerName(node);
    log(`copying gRPC certs from container ${name}`);

    const docker = await getDocker();
    const containers = await docker.listContainers();
    const info = containers.find(c => c.Names.includes(`/${name}`));
    if (!info) {
      log(`container not found, skipping cert copy`);
      return;
    }
    const container = docker.getContainer(info.Id);

    for (const { containerPath, hostPath } of certFiles) {
      if (!hostPath) continue; // skip if path is undefined
      ...
      try {
        ...
        stream.write(`cat ${containerPath}\n`);
        stream.write('exit\n');

        await new Promise<void>(resolve => stream.on('close', resolve));
        stream.destroy();

        // Extract the PEM content from the shell output.
        // The output includes the shell prompt and command echo, so we
        // extract just the content between BEGIN and END markers.
        const pemMatch = result.match(
          /-----BEGIN [A-Z ]+-----[\s\S]*?-----END [A-Z ]+-----/,
        );

        if (pemMatch) {
          await ensureDir(dirname(hostPath));
          await write(hostPath, pemMatch[0] + '\n');
          log(`copied ${containerPath} to ${hostPath}`);
        } else {
          log(`no PEM content found in ${containerPath}, skipping`);
        }
      } catch (err: any) {
        log(`failed to copy ${containerPath}: ${err.message}`);
      }
    }
  }
}
